...
- `make qtest` (`pytest tests --quick`), running only tests marked `@pytest.mark.quick` and only on pandas and polars: a representative sample across every test module in ~12s against ~95s for the full suite. It is the check to run while iterating, not before pushing — it deliberately skips the duckdb, dask and PySpark parametrizations, which is where backend-specific bugs live. Selecting nothing raises rather than reporting a vacuous pass, so a stale marker set fails loudly. `make ftest` is no longer described as the "fast" suite: with `qtest` alongside it, two targets sold on speed was one too many, and the 95s one had the word.
