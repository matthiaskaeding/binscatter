...
- `scipy>=1.11` is now a runtime dependency, alongside `narwhals`, `numpy` and `plotly`. The sparse min-norm solve behind multi-way absorption has no numpy equivalent, and scipy ships wheels everywhere and is already present in essentially every environment that installs a binscatter. Making it a plain dependency rather than an extra keeps there from being two supported configurations to test and one more install-time failure mode to document.
