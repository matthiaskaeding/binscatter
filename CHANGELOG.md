...
- `ci=` now works with absorbed fixed effects instead of always refusing. The sandwich variance genuinely needs the fixed-effect block — omitting it would not merely drop a term, it would contaminate the meat, since residuals are reconstructed from the fitted curve — so `inference.py` re-encodes the absorbed columns as dummies and reuses the existing interval machinery unchanged. Verified to match the one-hot path to 1e-8 for both interval styles. It is refused above `CI_MAX_FE_LEVELS` (500 total levels), where the bread and meat grow too large to be worth forming.
...
### Known limitations
- `ci=` is not available once the absorbed categorical controls span more than `CI_MAX_FE_LEVELS` (500) levels in total. The interval needs their contribution to the sandwich variance, and absorption exists precisely to never form it, so it is re-encoded as dummies — affordable only below that limit. The call raises `NotImplementedError` naming the columns and their cardinalities rather than returning an interval that quietly ignores the fixed effects.
- The degrees-of-freedom charge for absorbed fixed effects in the DPI selector uses `rank(D) = sum_f G_f - (F - 1)`, the convention `fixest` and `pyfixest` use, which assumes a connected design. A design that splits into disconnected components has lower rank, so this slightly overcharges and can nudge the selected bin count down. Estimates are unaffected — only bin selection.
