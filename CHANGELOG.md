...
- Added support for any number of categorical controls as absorbed fixed effects. The same controls now work with automatic bin selection and confidence intervals.
- Added a fast fixed effect benchmark that reports the test environment and can produce Markdown output.
- Added broader checks against `binsreg` and statsmodels for estimates, confidence intervals, automatic bin selection, polynomial fits, and categorical controls.
- Added numerical tests for changes in units, large offsets, collinearity, missing values, and difficult fixed effect designs.
- Added a pull request template.

### Changed

- All categorical controls now use fixed effect absorption, regardless of their number of levels. Constant categorical controls are still ignored.
- Fixed effect calculations now use grouped summaries, so memory use depends on observed group combinations rather than the number of rows.
- Renamed the test commands to `make test-fast`, `make test`, and `make test-spark`.
- The README now uses a small NHANES age and blood pressure example that runs without a network request. Generated README images now live under `artifacts/images/`.

### Fixed

- Fixed effect estimates no longer change when numeric controls use different units or when large constants are added (#107).
- Confidence intervals now work with absorbed fixed effects and avoid collisions with user column names.
- Robust bias corrected intervals no longer show the warning intended for pointwise intervals.
- Invalid inputs now give clear errors for unusable columns, reserved column names, and noninteger bin counts.
- PySpark tests no longer start Spark during the default test run.

### Removed

- Removed `ABSORB_MIN_LEVELS`, because categorical controls no longer switch between indicator columns and absorption.

### Known limitations

- With three or more absorbed factors, nested factors can make the degrees of freedom adjustment slightly conservative. Point estimates are unaffected, and one or two factors remain exact.
