...
- Three consistently named test targets replace the ambiguous `qtest` and `ftest` names: `make test-quick` runs the representative sample, `make test` runs the suite without PySpark, and `make test-pyspark` runs the full backend matrix. The quick sample runs tests marked `@pytest.mark.quick` on pandas, Polars and DuckDB, including direct comparisons with the paper authors' `binsreg` package; it skips the distributed Dask and PySpark parametrizations. Selecting nothing raises rather than reporting a vacuous pass, so a stale marker set fails loudly.
- `tests/test_fe_contract.py`, the fixed-effect guarantees that do not depend on how fixed effects are implemented: the dots equal a dense one-hot OLS fit at average controls, for one, two and three factors, with and without numeric controls; they are unmoved by row order, level labels, integer-versus-string spelling of an identifier, and a constant factor; nested factors, singleton levels and two factors sharing label values all hold; nulls drop their rows; every backend agrees; and 5,000 levels finish in seconds rather than taking a quadratic route. It imports only `binscatter()`, patches nothing, and asserts nothing about which path ran — verified by running it with absorption disabled entirely, where everything but the cost test still passes. The existing two fixed-effect files stay as regression tests for the current design, which they test by forcing its two routes and comparing them.
- Tests that specify behaviour binscatter does not have yet are marked `xfail(strict=True)` and listed in `KNOWN_FAILURES` in `tests/conftest.py`, with the gap tracked in #95. Eighty-nine of them: the normal equations are assembled from raw uncentered moments, so an automatic selector is not invariant to shifting `x` (106 bins against 41 on the same data), large integer controls are off by multiples and near-collinear ones by a third; several designs that cannot identify their coefficients are accepted rather than refused; a control column named after an internal aggregation alias crashes; and an all-null column reaches `quantiles.py` as `float(None)`. Strict marking means each one turns red the moment it starts passing, so the list is a work queue that cannot drift out of date. One test per group carries the `quick` mark, so `make test-quick` reports the gap rather than a clean bill of health.
...
### Fixed
- A control column named after an internal aggregation temporary (`__fe_count`, `__mfe_resp_0`) crashed with `cannot insert __fe_count, already exists`, which named neither the column nor the cause. The fixed-effect collectors now step aside from names the frame already carries, the way `binscatter` already suffixes its bin column (#95).
