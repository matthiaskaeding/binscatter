...
### Known limitations
- The degrees-of-freedom charge for absorbed fixed effects, in both the DPI selector and the interval sandwich, uses `rank(D) = sum_f G_f - (F - 1)` — the convention `fixest` and `pyfixest` use, which assumes a connected design. A design that splits into disconnected components has lower rank, so this slightly overcharges: the selected bin count can be nudged down, and standard errors are marginally conservative. Point estimates are unaffected.
