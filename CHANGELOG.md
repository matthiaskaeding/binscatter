...
- Requesting `ci="pointwise"` while the bin count comes from an automatic selector now warns. ROT and DPI minimise IMSE, which is exactly the regime where an interval that ignores the smoothing bias under-covers, so a reliable pointwise interval needs `num_bins` passed explicitly — or `ci="rbc"`, which corrects the bias instead.
