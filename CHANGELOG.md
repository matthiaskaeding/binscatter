...
- `tests/test_sparse_fe.py`, covering multi-way fixed-effect absorption (#74): equivalence against the one-hot path and an independent dense-OLS oracle, both for two and for three absorbed factors; nested factors (county in state) and singleton-heavy panels, which are the degenerate cases for a group-mean method; integer-coded factors via `categorical=`; the routing decisions, including that the DPI selector is never handed more than one factor; the crosstab budget guard; the collected moment blocks against a dense dummy matrix; and the fallback to single-factor absorption when scipy is absent, asserted to give identical results. Absorbed bin values match dense one-hot OLS to ~4e-14. Unlike `tests/test_fixed_effects.py`, these run on pandas, polars, duckdb and dask but not PySpark: multi-way absorption reaches the backend only through `group_by`, whose per-factor aggregates and two-key crosstabs the one-way absorption tests already exercise on PySpark, and everything past those aggregates is numpy and scipy on the driver.
