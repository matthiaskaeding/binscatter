...
- `tests/test_sparse_fe.py`, covering multi-way fixed-effect absorption (#74): equivalence against the one-hot path and an independent dense-OLS oracle, both for two and for three absorbed factors and both on every backend; nested factors (county in state) and singleton-heavy panels, which are the degenerate cases for a group-mean method; integer-coded factors via `categorical=`; the routing decisions, including that the DPI selector is never handed more than one factor; the crosstab budget guard; and the collected moment blocks against a dense dummy matrix; and the fallback to single-factor absorption when scipy is absent, asserted to give identical results. Absorbed bin values match dense one-hot OLS to ~4e-14.
